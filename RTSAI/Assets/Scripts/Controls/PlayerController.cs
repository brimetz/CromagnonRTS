...
public sealed class PlayerController : UnitController
{
    ...
    override protected void Start()
    {
        ...
        {
            Factory Fact = null;
            if (RequestFactoryBuild(WantedFactoryId, floorPos, ref Fact))
            {
    ...
    void StartSelection()
    {
        ...
        else if (Physics.Raycast(ray, out raycastInfo, Mathf.Infinity, unitMask))
        {
            ...
            {
                if (isShiftBtPressed && selectSquad != null)
                {
                    selectSquad.RemoveUT(selectedUnit);
                    UnselectUnit(selectedUnit);

                    if (!selectedUnit.IsInSquad() || selectSquad.Count <= 1)
					{
                        PlayerMenuController.ShowFormationMenu(false);
					}
                    if (selectSquad.Count == 0)
                    {
                        selectSquad = null;
                        PlayerMenuController.ShowPostureMenu(false);
                        return;
                    }
                }
                else if (isCtrlBtPressed)
                {
                    if (selectSquad == null)
                    {
                        ...
                        NewSquad.SetTeam(ETeam.Blue);
                        selectSquad = NewSquad;
                    }
                    if (!selectSquad.Units.Contains(selectedUnit))
                        selectSquad.Units.Add(selectedUnit);
                ...
                else
                {
                    ...
                    NewSquad.AddUT(selectedUnit);
                    NewSquad.SetSelected(true);

                    if (selectSquad != null)
                        selectSquad.SetSelected(false);

                    selectSquad = NewSquad;
                    SelectUnit(selectedUnit);
                }

                if (selectSquad != null && selectSquad.Count > 0)
                {
                    PlayerMenuController.ShowPostureMenu(true);
                    PlayerMenuController.SquadMenuCanvas.UpdatePostureUI(selectSquad);
                    if (selectSquad.Count > 1)
                    {
                        PlayerMenuController.ShowFormationMenu(true);
                        PlayerMenuController.SquadMenuCanvas.UpdateFormationUI(selectSquad);
                    }
                }
                else
                    PlayerMenuController.ShowSquadMenu(false);
            }
        }
        else if (Physics.Raycast(ray, out raycastInfo, Mathf.Infinity, floorMask))
        {
            ...
            SelectionStart.y = raycastInfo.point.y + 1f;
    ...
    void EndSelection()
    {
        ...
        Vector3 size = Vector3.up * 2f * SelectionBoxHeight + SelectionEnd - SelectionStart;
        ...
        foreach (Collider col in colliders)
        {
            ...
            if (selectedEntity.GetTeam() == GetTeam())
            {
                ...
                {
                    if ((selectedEntity as Unit).OwnSquad != null)
                        NewSquad.currentFormation = (selectedEntity as Unit).OwnSquad.currentFormation;
                    NewSquad.AddUT(selectedEntity as Unit);
        ...
        if (NewSquad.Count > 0)
        {
            ...
            if (NewSquad.Count > 1)
            {
                PlayerMenuController.ShowSquadMenu(true);
                PlayerMenuController.SquadMenuCanvas.UpdateFormationUI(selectSquad);
            }
            else
                PlayerMenuController.ShowPostureMenu(true);
        ...
        if (selectSquad != null)
            PlayerMenuController.ShowFactoryMenu(false);
        else
            PlayerMenuController.ShowSquadMenu(false);
    ...
    {
        selectSquad.currentFormation = EFormation.Line;
        selectSquad.UpdateOffset();
        PlayerMenuController.SquadMenuCanvas.UpdateFormationUI(selectSquad);
    }

    public void FormationColumn()
    {
        selectSquad.currentFormation = EFormation.Column;
        selectSquad.UpdateOffset();
        PlayerMenuController.SquadMenuCanvas.UpdateFormationUI(selectSquad);
    }

    public void FormationSquare()
    {
        selectSquad.currentFormation = EFormation.Square;
        selectSquad.UpdateOffset();
        PlayerMenuController.SquadMenuCanvas.UpdateFormationUI(selectSquad);
    }
    ...
    {
        selectSquad.currentFormation = EFormation.Triangle;
        selectSquad.UpdateOffset();
        PlayerMenuController.SquadMenuCanvas.UpdateFormationUI(selectSquad);
    }

    public void FormationUnstructured()
    {
        selectSquad.currentFormation = EFormation.Unstructured;
        PlayerMenuController.SquadMenuCanvas.UpdateFormationUI(selectSquad);
    }

    public void PostureAggressive()
	{
        selectSquad.SetPosture(EPosture.Aggressive);
        PlayerMenuController.SquadMenuCanvas.UpdatePostureUI(selectSquad);
    }

    public void PosturePassive()
    {
        selectSquad.SetPosture(EPosture.Passive);
        PlayerMenuController.SquadMenuCanvas.UpdatePostureUI(selectSquad);
    }

    public void PostureRepair()
    {
        selectSquad.SetPosture(EPosture.Repair);
        PlayerMenuController.SquadMenuCanvas.UpdatePostureUI(selectSquad);
    }
    #endregion
    ...
    public void ExitFactoryBuildMode()
    {
    ...
    void ComputeUnitsAction()
    {
        ...
        if (Physics.Raycast(ray, out raycastInfo, Mathf.Infinity, damageableMask))
        {
            ...
            if (other != null)
            {
                ...
                {
                    selectSquad.SetAttackTarget(other);
                    SetTargetCursorPosition(other.transform.position);
                ...
                {
                    selectSquad.SetRepairTarget(other);
                    SetTargetCursorPosition(other.transform.position);
        ...
        else if (Physics.Raycast(ray, out raycastInfo, Mathf.Infinity, targetMask))
        {
            ...
            {
                selectSquad.SetCaptureTarget(target);
        ...
        else if (Physics.Raycast(ray, out raycastInfo, Mathf.Infinity, floorMask))
        {
            ...
            selectSquad.SetPositionTarget(newPos);
